...
async def get_cart(
    ...
    # Явное преобразование ORM-объектов в Pydantic схемы
    cart_items = [CartItemSchema.model_validate(item) for item in items]
    
    return CartSchema(
        user_id=current_user.id,
        items=cart_items,  # Используем преобразованный список
